...
public class ConsoleLibrary {
    static Connection connection; // Класс java.sql.Connection представляет в JDBC сеанс работы с базой данных.
    static BufferedReader consoleReader = new BufferedReader(new InputStreamReader(System.in)); //  для чтения ввода пользователя из консоли
...
//            DatabaseInitializer.initializeDatabase(connection);
        ...
        try {
            System.out.print(
                    "Выберите действие для Домашней библиотеки\n" +
                            ...
                            "13 Вывести авторов на указанном этаже в лексикографическом порядке\n" +
                            "14 Вывести издания, для которых не указан год написания\n" +
                            ">> ");
    ...
    private static void doActionInDataBase(int action) {
        try {
            ...
            } else if (action == MenuActions.SHOW_AUTHORS_IN_PLACE.getAction()) {
                showAuthorsOnFloorInLexicOrder();
            } else if (action == MenuActions.SHOW_PUBLICATION_WITHOUT_YEAR.getAction()) {
                showPublicationsWithoutYearWrite();
            } else {
    ...
    private static void showAuthorsOnFloorInLexicOrder() throws IOException {
        ...
        String authors = sqlRequests.showAuthorsOnFloorInLexicOrder(floor);
        if (!authors.isEmpty()) {
            System.out.println("Авторы на этаже " + floor + ":\n" + authors);
        } else {
            System.out.println("Авторы не найдены на указанном этаже!");
        }
    }

    private static void showPublicationsWithoutYearWrite() throws IOException {
        SQLRequests sqlRequests = new SQLRequests(connection);
        System.out.println(sqlRequests.showPublicationsWithoutYearWrite());
    }
    ...
    private static void replaceWithDefaultDataFromFile() {
        SQLRequests sqlRequests = new SQLRequests(connection);
        FileDataHandler fileHandler = new FileDataHandler("D:\\KAI\\JAVA\\labs\\lab9\\JavaJDBC-master\\src\\main\\java\\org\\example\\DataFile.txt");
    ...
    private static void updateBook() throws IOException {
        ...
        System.out.print("Введите новый год написания (оставьте пустым, если не указано)\n>> ");
        String yearWriteInput = consoleReader.readLine();
        Integer yearWrite = yearWriteInput.isEmpty() ? null : Integer.parseInt(yearWriteInput);
        System.out.print("Введите новый вес в граммах\n>> ");
        int weight = Integer.parseInt(consoleReader.readLine());
        System.out.println(sqlRequests.updateBook(id, author, publication, publishingHouse, yearPublic, pages, yearWrite, weight, placeId));
    }
    ...
    private static void addNewBookInTableDB() throws IOException {
        ...
        int pages = Integer.parseInt(consoleReader.readLine());
        System.out.print("Введите год написания (оставьте пустым, если не указано)\n>> ");
        String yearWriteInput = consoleReader.readLine();
        Integer yearWrite = yearWriteInput.isEmpty() ? null : Integer.parseInt(yearWriteInput);
        ...
        System.out.print("Введите id места, где будет находиться книга\n>> ");
        int placeId = Integer.parseInt(consoleReader.readLine());
        System.out.println(sqlRequests.createNewRowInBookTable(author, publication, publishingHouse, yearPublic, pages, yearWrite, weight, placeId));
    }
